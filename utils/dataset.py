...
class Makedataset():
	...
	def mask_dataset(self, name, product_list, batchsize, shuf = True):
		if name == "mvtec" and self.train_mode == "zero":
			...
			dataset = OtherDataset(root=self.train_data_path, transform=self.preprocess_test, target_transform=self.target_transform, 
											   mode =self.mode, train_mode=self.train_mode, product_list= product_list, dataset= name)
