...
def test(args):
    ...
    imgClsHead.load_state_dict(checkpoint["ImageClsHead"], strict=False)
    model_tf.load_state_dict(checkpoint["ParallelThenRefine"], strict=False)
    ...
    prompt_ceshi.build_state_pool(model,device)
    ...
    for items in test_dataloader:
        ...
        results['imgs_masks'].append(gt_mask.squeeze(1).numpy())  
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            text_embeddings,normal_state,abnormal_state  = prompt_ceshi.forward_ensemble(model,model_tf,image_features,patch_tokens, class_token,cls_name, device)
            ...
            for layer in range(len(patch_tokens_linear)):
                ...
                anomaly_maps_raworin.append(anomaly_map_raw)
                ...
                anomaly_maps_rawo = torch.softmax(anomaly_map_raw, dim =1)
            ...
            for layer in range(len(patch_tokens)):
                ...
                dense_feature = dense_feature /  dense_feature.norm(dim=-1, keepdim = True)
                priormask =  anomaly_maps_raworin[layer].clone() 
                anomaly_map_new = UFAMTemplate(text_embeddings.permute(0,2,1), dense_feature, anomaly_prior=priormask)  
                B, L, C = anomaly_map_new.shape 
                H = int(np.sqrt(L))
                anomaly_map_new = F.interpolate(anomaly_map_new.permute(0, 2, 1).view(B,2,H,H),
                                            size = img_size, mode = 'bilinear', align_corners=True)
                anomaly_maps_logits.append(torch.softmax(anomaly_map_new, dim =1))
                anomaly_map_new = torch.softmax(anomaly_map_new, dim =1)[:, 1, :, :]
                anomaly_maps_new.append(anomaly_map_new.cpu().numpy())

            pro_img=imgClsHead(text_embeddings_orin,image_features,patch_tokens,anomaly_maps_logits,state)
            text_probs = F.softmax(pro_img, dim=1)   
            text_probs = text_probs[:, 1:2].squeeze(1)      
            results['pro_img'].append(text_probs.cpu().numpy())
...
def calcuate_metric(results, obj_list, logger, alpha = 0.2, args = None):
    ...
    pro_img=np.concatenate(results['pro_img'], axis=0)
    ...
    for obj in obj_list: 
        ...
        pr_sp = pro_img[object_index].copy()   
        pr_px =  normalize(pr_px_2) 
        ...
        pr_sp_tmp = np.mean(pr_sp_2, axis=1)+pr_sp
        ...
        print("{}--->  iou:{} aupro_px:{} auroc_sp:{} f1:{}  threshold:{}, ".format(obj,iou,aupro_px,auroc_sp,f1_sp,best_threshold))
