...
class DTD_dataset():
    def __init__(self,path_root):
        ...
        self.dataset_map = {"Woven_127": "woven1", "Woven_125": "woven2","Woven_104": "woven3","Woven_068": "woven4","Woven_001": "woven5", "Stratified_154": "stratified", "Perforated_037": "perforated",
                            "Mesh_114": "mesh", "Matted_069": "matted1", "Marbled_078": "marbled2", "Fibrous_183": "fibrous", "Blotchy_099": "blotchy"}
            ...
            for mode in ["train","test"]:
                ...
                if len(defect_classes) == 1 and "good" in defect_classes and mode=="train":
                    ...
                    for i in range(len(data_list)):
                        ...
                        save_img_path = os.path.join(des_path_root,new_class_name,"train","good","DTD_"+str(img_name_id)+ "_" + str(id).zfill(6)+".bmp")  
                        ...
                        for classes in defect_classes:
                            ...
                            if classes == "good":
                                for i in range(len(data_list)):
                                    ...
                                    raw_img = cv2.imread(data_list[i],cv2.IMREAD_COLOR)
                                    new_class_name = self.dataset_map[data_name]
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","good","DTD_"+"good_"+str(img_name_id)+ "_" +str(id).zfill(6)+".bmp") 
                                    cv2.imwrite(save_img_path,raw_img)
                                    id = id + 1
                            else:
                                for i in range(len(data_list)):
                                    ...
                                    new_class_name = self.dataset_map[data_name]
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","anomaly",f"DTD_{classes}_"+str(img_name_id)+ "_" +str(id).zfill(6)+".bmp")
                                    save_mask_path = os.path.join(des_path_root,new_class_name,"ground_truth","anomaly",f"DTD_{classes}_"+str(img_name_id)+ "_" +str(id).zfill(6)+".png")
                                    cv2.imwrite(save_img_path,raw_img)
