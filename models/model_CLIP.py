...
class CLIP(nn.Module):
    def __init__(self,
        ...
        k=4
        num_tokens=4
        self.W_normal = nn.Parameter(torch.eye(k, num_tokens))
        self.W_abnormal = nn.Parameter(torch.eye(k, num_tokens))
    ...
    def encode_text_origanl(self, text):
        x = self.token_embedding(text).type(self.dtype) 
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2) 
        x,attns, tokens = self.transformer(x)
        x = x.permute(1, 0, 2)  
    ...
    def encode_text(self,context_word, text, visual_feature,B_normal,B_abnormal, out_layers = [2,5,8,12]):  

        pos_x_context, pos_y_context = torch.where(text == 13089) 
        pos_x_state, pos_y_state = torch.where(text == 1295)
        pos_x_class, pos_y_class = torch.where(text == 1874) 
        x = self.token_embedding(text).type(self.dtype)  
        N, L, D = x.shape

        if B_normal !=None:
            state = torch.matmul(B_normal, self.W_normal).transpose(1, 2)  
        if B_abnormal !=None:
            state = torch.matmul(B_abnormal, self.W_abnormal).transpose(1, 2)  

        if context_word!=None:
            prompt_word= torch.cat([context_word, state, visual_feature], dim=1)

        text_feature_list = []
        N=1
        for i in range(visual_feature.shape[0]):
            x_new = torch.zeros(1, x.shape[0], x.shape[1]).to(x.device)
            class_idx = pos_y_context[i]
            part1 = x[i, 0:class_idx, :]                   
            part4 = prompt_word[i, :, :]                   
            part5 = x[i, class_idx+1:(self.context_length - prompt_word.shape[1]  + 1), :]  
            x_new = torch.cat([part1, part4, part5], dim=0).unsqueeze(0)           
...
if __name__ == '__main__':
    ...
    image1 = torch.rand((1,3,140,140),dtype = torch.float32).to(device)   
