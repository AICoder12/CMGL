...
from models.prompt_ensemble import LCBlock
from models.ImagelClssHead import ImagelClssHead
...
def train(args):
    ...
    Product_groups = Split_Product(args.dataset) 
    ...
    imgClsHead=ImagelClssHead(model_configs['vision_cfg']['width'],model_configs['text_cfg']['width'],features_list).to(device)
    imgClsHead.train()
    loss_cross = nn.CrossEntropyLoss()
    model_tf = LCBlock(refine_type="transformer").to(device)
    model_tf.train()

    prompt_pre = Prompt_Ensemble(args.prompt_len,tokenizer)  
    UFAMTemplate = UAFM(dim_v = model_configs["vision_cfg"]['width'], dim_t =  model_configs['text_cfg']['width'], dim_out= model_configs["vision_cfg"]['width']).to(device)
    UFAMTemplate.train()   
    ...
    group1 = []
    group2 = []
    for n, m in UFAMTemplate.named_parameters():
        if n not in ["prompt_temp_l1"]:
            group1.append(m)
        else:
            group2.append(m)
            print(n)
    ...
    for group_id in range(len(Product_groups)):
        ...
        if args.resume_checkpoint_path is not None: 
            ...
            UFAMTemplate.load_state_dict(checkpoint["UFAMTemplate"], strict=False)
            New_Lan_Embed.load_state_dict(checkpoint["New_Lan_Embed"])
            imgClsHead.load_state_dict(checkpoint["ImageClsHead"])
            model_tf.load_state_dict(checkpoint["ParallelThenRefine"])
            _load_stages(model, checkpoint, "prompt")

        prompt_pre.build_state_pool(model,device)
        ...
        for epoch in range(epochs):
            ...
            for items in post_dataloader:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    text_embeddings,normal_state,abnormal_state = prompt_pre.forward_ensemble(model,model_tf,image_features,patch_tokens, class_token,cls_name, device)
                    text_embeddings_orin=text_embeddings.clone()
                    ...
                    for layer in range(len(patch_tokens_linear)):
                        ...
                        anomaly_maps_orin.append(anomaly_map_raw)
                    ...
                    for layer in range(len(patch_tokens)):
                        ...
                        priormask =  anomaly_maps_orin[layer].clone() 
                        anomaly_map_new = UFAMTemplate(text_embeddings.permute(0,2,1), dense_feature, anomaly_prior=priormask,epoch=epoch)
                    ...
                    pro_img=imgClsHead(text_embeddings_orin,image_features,patch_tokens,anomaly_maps_new,state)
                    loss_class = loss_cross(pro_img, anomaly)
                    margin=0.5
                    sim = F.cosine_similarity(normal_state, abnormal_state, dim=-1)  
                    loss_sep = torch.clamp(sim - margin, min=0).mean()

                gt = items['img_mask'].squeeze().to(device)
                gt[gt > 0.5], gt[gt< 0.5] = 1, 0

                loss_new = torch.zeros(1, device=device)
                ...
                loss_base = torch.zeros(1, device=device)
                ...
                loss = loss_new + loss_base+loss_sep+loss_class
            ...
            # ap_new = evaluate_post(val_post_dataloader, model, trainable_layer, New_Lan_Embed, UFAMTemplate, prompt_pre, device, args, val_obj_list_post)   
            ...
            if (epoch + 1) % args.print_freq == 0:
                logger.info('epoch [{}/{}], group_id: {} loss:{:.4f}  loss_base:{:.4f}   loss_new:{:.4f} '.format(epoch + 1, epochs, group_id, np.mean(loss_list),np.mean(loss_raw_list),np.mean(loss_new_list)))
            #---------------------------- End evaluation ----------------------------------------#
  
            if (epoch + 1) % args.save_freq == 0:
                ckp_path = os.path.join(save_path, 'epoch_' + str(epoch + 1) + '_group_id_' + str(group_id) + '.pth')
                save_dict = {'trainable_linearlayer': trainable_layer.state_dict(), 'New_Lan_Embed': New_Lan_Embed.state_dict(), "UFAMTemplate":UFAMTemplate.state_dict(),"ImageClsHead":imgClsHead.state_dict(),"ParallelThenRefine":model_tf.state_dict()}
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser("CMGL", add_help=True)
    # path
